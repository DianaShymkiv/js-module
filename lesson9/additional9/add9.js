...
for (const user of usersList) {
    ...
    userWrap.style.backgroundColor = 'lightGrey';
    userWrap.style.marginBottom = '10px';
    userWrap.style.border = '5px solid darkGrey';

    userWrap.classList.add('user');

    recCreateDiv(user, userWrap);

    document.body.appendChild(userWrap);
}


function recCreateDiv(user, userWrap) {
    for (const key in user) {
        if (typeof user[key] === 'object' && user[key] !== null) {
            ...
            recCreateDiv(user[key], div);

            userWrap.appendChild(div);
        } else {
            let userText = document.createElement('div');
            userText.innerHTML = `<b>${key}:</b> ${user[key]}`;
